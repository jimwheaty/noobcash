...
class Node:
	...
	def create_genesis_block(self):
		t = Transaction(0, self.wallet.public_key, NUMBER_OF_NODES * STARTING_NBC_PER_NODE, [])
		utxo = Transaction_Output(self.wallet.public_key, NUMBER_OF_NODES * STARTING_NBC_PER_NODE, t.transaction_id) 
	...
	def initialize_network(self):
		...
		for peer in self.ring.values():
			if peer.public_key != self.wallet.public_key:
				t = self.create_transaction(peer.public_key, STARTING_NBC_PER_NODE)
